...
public:
    ...
    bool operator ==(const Rational& other) const {
        ...
        if (chis == 0 and other.chis == 0) return true;

        if (sgn != other.sgn) return false;

        if (znm == 0 and other.znm == 0) return true;

        return chis == other.chis and znm == other.znm;
    }
    ...
    Rational operator +(const Rational& other) const {
        ...
        if (znm == 0 and other.znm == 0) {

            if (sgn == other.sgn) return other;

            return Rational(0, 0);
        }
        if (znm == 0) return *this;

        if (other.znm == 0) return other;

        int min = Rational::min(znm, other.znm);

        int firstNum = chis * (min / znm);

        int secondNum = other.chis * (min / other.znm);
        ...
        Rational result(resultNum, min);
        result.sgn = resultSgn;
        return result;
    }

    Rational operator *(const Rational& other) const {
        Rational result(chis * other.chis,
            znm * other.znm);
    ...
    Rational operator /(const Rational& other) const {
        auto result = Rational(chis * other.znm,
            znm * other.numerator());
        result.sgn = sgn * other.sgn;
        return result;
    }
    ...
    operator double() const {
        return sgn * chis / double(znm);
    }

    operator bool() const {
        return chis != 0 or znm == 0;
    }
